(function () {
  ...
  const comboFamilies = {
    ...
    Kickboxing: {
      ...
      Defense: [
        ...
        ['jab', 'switch stance', 'rear body kick'],
      ...
      Freestyle: [
        ...
        ['jab', 'cross', 'switch stance', 'rear low kick'],
      ],
    },
  };
  ...
  function stanceSwitchEnabled() {
    return runtime.currentSession ? Boolean(runtime.currentSession.allowSwitchStance) : Boolean(runtime.state.setup.allowSwitchStance);
  }

  function mapActionToStance(action, stance) {
    const leadSide = stance === 'Southpaw' ? 'right' : 'left';
    const rearSide = stance === 'Southpaw' ? 'left' : 'right';
    const mappings = {
      'lead hook': `${leadSide} hook`,
      'rear hook': `${rearSide} hook`,
      'lead uppercut': `${leadSide} uppercut`,
      'rear uppercut': `${rearSide} uppercut`,
      'lead low kick': `${leadSide} low kick`,
      'rear low kick': `${rearSide} low kick`,
      'lead body kick': `${leadSide} body kick`,
      'rear body kick': `${rearSide} body kick`,
      'lead head kick': `${leadSide} head kick`,
      'rear head kick': `${rearSide} head kick`,
      'lead knee': `${leadSide} knee`,
      'rear knee': `${rearSide} knee`,
      'lead teep': `${leadSide} teep`,
      'rear teep': `${rearSide} teep`,
      'lead elbow': `${leadSide} elbow`,
      'rear elbow': `${rearSide} elbow`,
      'lead hook to body': `${leadSide} hook to body`,
      'lead hook to head': `${leadSide} hook to head`,
    };
    return mappings[action] || action;
  }

  function formatComboForStance(comboText, startingStance) {
    const actions = comboText
      .split(',')
      .map((item) => item.trim().toLowerCase())
      .filter(Boolean);
    let activeStance = startingStance || 'Orthodox';
    const displayActions = actions.map((action) => {
      if (action === 'switch stance') {
        activeStance = activeStance === 'Orthodox' ? 'Southpaw' : 'Orthodox';
        return 'switch stance';
      }
      return mapActionToStance(action, activeStance);
    });
    return {
      text: displayActions.join(', '),
      endingStance: activeStance,
    };
  }
  ...
  function renderSegments() {
    ...
    createSegmentButtons(els.stanceSegment, segmentDefinitions.stance, 'stance');
  ...
  function renderSetupForm() {
    ...
    els.allowSwitchStanceInput.checked = Boolean(runtime.state.setup.allowSwitchStance);
  ...
  function familyPool() {
    ...
    const switchMoves = new Set(['switch stance', 'switch kick', 'switch knee']);
    const filteredForSwitching = stanceSwitchEnabled()
      ? basePool
      : basePool.filter((combo) => combo.every((move) => !switchMoves.has(move)));
    const allowedMoves = currentAllowedMoves();
    if (!allowedMoves) return filteredForSwitching;
    const filtered = filteredForSwitching.filter((combo) => combo.every((move) => allowedMoves.has(move) || bodyAttackMoves.includes(move)));
    return filtered.length
      ? filtered
      : comboFamilies[mode].Balanced
          .filter((combo) => combo.every((move) => !switchMoves.has(move)))
          .filter((combo) => combo.every((move) => allowedMoves.has(move) || bodyAttackMoves.includes(move)));
  }
  ...
  function currentComboPools() {
    ...
    const switchMoves = new Set(['switch stance', 'switch kick', 'switch knee']);
    const filteredAttackPool = stanceSwitchEnabled()
      ? defaultAttackPool
      : defaultAttackPool.filter((move) => !switchMoves.has(move));
    const filteredDefensePool = stanceSwitchEnabled()
      ? defaultDefensePool
      : defaultDefensePool.filter((move) => !switchMoves.has(move));
  ...
  function startSession() {
    runtime.currentSession = {
      ...
      phase: 'work',
      round: 1,
      currentStance: runtime.state.setup.stance,
  ...
  function formatCoachCallout(comboText, leadIn) {
    const replacements = {
      ...
      'lead hook to head': 'left hook head',
  ...
  function updateLiveCombo(initial) {
    ...
    els.comboPreview.textContent = `Hold this combo for the round | ${session.currentStance} stance`;
  ...
  function savePreset() {
    ...
    const preset = {
      ...
      mode: setup.mode,
      stance: setup.stance,
      allowSwitchStance: setup.allowSwitchStance,
      rounds: setup.rounds,
  ...
  function applyPreset(id) {
    ...
    runtime.state.setup = {
      ...
      mode: preset.mode,
      stance: preset.stance || 'Orthodox',
      allowSwitchStance: Boolean(preset.allowSwitchStance),
  ...
  function startCareerSession() {
    ...
    runtime.state.setup = {
      ...
      stance: runtime.state.setup.stance || 'Orthodox',
      allowSwitchStance: progress.chapter.minSessions >= 6,
  ...
  function registerEvents() {
    ...
    els.allowSwitchStanceInput.addEventListener('change', () => {
      playButtonGong();
      runtime.state.setup.allowSwitchStance = els.allowSwitchStanceInput.checked;
      runtime.state.setup.careerMode = false;
      saveState();
    });
